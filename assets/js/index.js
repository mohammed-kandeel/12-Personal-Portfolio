...
function portfolioBtnStyle(item) {
  for (const btn of portfolioBtns) {
    ...
    btn.classList.add("bg-white", "dark:bg-slate-800", "text-slate-600", "dark:text-slate-300", "hover:bg-slate-100", "dark:hover:bg-slate-700", "border", "border-slate-300", "dark:border-slate-700");
  }
  item.classList.add("bg-linear-to-r", "from-primary", "to-secondary", "text-white", "hover:shadow-lg", "hover:shadow-primary/50");
  item.classList.remove("bg-white", "dark:bg-slate-800", "text-slate-600", "dark:text-slate-300", "hover:bg-slate-100", "dark:hover:bg-slate-700", "border", "border-slate-300", "dark:border-slate-700");
}
...
function slideTestimonials({ moveTo = 1, index = -1 }) {
  const viewWidth = Math.floor(+getComputedStyle(testimonials).width.replace("px", ""));
  const cardWidth = Math.ceil(+getComputedStyle(testimonialsCards[0]).width.replace("px", ""));
  let maxMove = Math.ceil(testimonialsCards.length * cardWidth - (viewWidth / cardWidth) * cardWidth);
  index === -1 ? (index = 1) : (moveTestimonialsCards = 0);
  moveTestimonialsCards += Math.floor(cardWidth * moveTo * index);
  if (moveTestimonialsCards > maxMove) moveTestimonialsCards = 0;
  else if (moveTestimonialsCards < 0) moveTestimonialsCards = maxMove;
  testimonials.style.transform = `translate(${moveTestimonialsCards}px)`;
  let carouselIndex = Math.ceil((moveTestimonialsCards - 2) / cardWidth);
  changeCarouselIndicatorStyle(carouselIndex);
}
...
function createBtnsThemeColors() {
  ...
  for (let i = 0; i < themeColors.length; i++) {
    ...
    btn.classList.add("w-12", "h-12", "rounded-full", "cursor-pointer", "transition-transform", "hover:scale-110", "border-2", "border-slate-200", "dark:border-slate-700", "hover:border-primary", "shadow-sm");
